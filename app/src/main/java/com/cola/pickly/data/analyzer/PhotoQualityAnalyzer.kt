...
class PhotoQualityAnalyzer(
        ...
        try {
            ...
            if (validFaces.isEmpty()) {
                ...
                return@withContext RecommendationScore(
                    ...
                    rawSharpness = sharpnessRaw,
                    // 작은 얼굴들이라도 박스는 표시해줌
                    allFaceBoundingBoxes = getScaledRects(allDetectedFaces, originalSize, bitmap.width, bitmap.height)
                )
            }
            ...
            if (isFaceCropped(bestFace, bitmap.width, bitmap.height)) {
                ...
                return@withContext RecommendationScore(
                    ...
                    faceBoundingBox = getScaledRect(bestFace.boundingBox, originalSize, bitmap.width, bitmap.height),
                    allFaceBoundingBoxes = getScaledRects(validFaces, originalSize, bitmap.width, bitmap.height)
                )
            }
    ...
    // 좌표 변환 헬퍼 함수
    private fun getScaledRect(box: Rect, originalSize: Size, bitmapWidth: Int, bitmapHeight: Int): Rect {
    ...
    private fun getScaledRects(faces: List<Face>, originalSize: Size, bitmapWidth: Int, bitmapHeight: Int): List<Rect> {
        return faces.map { getScaledRect(it.boundingBox, originalSize, bitmapWidth, bitmapHeight) }
    }
    ...
    private fun calculateScore(
        ...
        val bestRect = Rect(
            (bestBox.left * scaleX).toInt(),
            (bestBox.top * scaleY).toInt(),
            (bestBox.right * scaleX).toInt(),
            (bestBox.bottom * scaleY).toInt()
        )

        val allRects = allValidFaces.map { face ->
            val box = face.boundingBox
            Rect(
                (box.left * scaleX).toInt(),
                (box.top * scaleY).toInt(),
        ...
        return RecommendationScore(
            ...
            isCutoff = false,
            faceBoundingBox = bestRect,
            allFaceBoundingBoxes = allRects,
