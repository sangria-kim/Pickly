...
import com.cola.pickly.data.database.PhotoScoreEntity
 ...
 */
class PickBestPhotosUseCase(
    private val analyzer: PhotoQualityAnalyzer,
    private val photoScoreDao: PhotoScoreDao
) {

    suspend operator fun invoke(groups: List<WeeklyGroup>): List<WeeklyGroup> = coroutineScope {
        // 주(Group) 단위로 병렬 처리
